"""Stage 10 (v5): single-set agentic optimization — no dev/test split.

Design (per user direction):
  - ONE official evaluation set per neuron = the repo protocol set
    (top-50 activating + 50 seeded random zero-activation texts). The reported
    metric is correlation on this set; the optimizer may query it directly.
  - The agent may ALSO annotate any sample of texts it chooses (indices arg),
    e.g. to validate a candidate on self-picked examples before spending on the
    official set. Labels for arbitrary samples: activation > 0.
  - Budget: total annotator calls per neuron (default 6000), not "score calls".
  - Final pick: argmax official-set correlation among officially-scored
    candidates (baseline description is always scored first, so it is the floor).
  - Objective: pure hill-climbing to the structural ceiling (perfect
    discrimination) on the official set.
"""
...
from agentic_interp.harness import (load_artifacts, select_neurons, test_indices,
                                    score_description, compute_metrics, ART,
                                    TASK_SPECIFIC_INSTRUCTIONS, DATASET_DESC,
                                    ANNOTATOR_MODEL, MAX_WORDS, AGENT_RUN_DIRNAME)
...
OFFICIAL METRIC: an annotator LLM is shown (text, description) pairs and asked yes/no "does this TEXT satisfy this PROPERTY?" on a fixed evaluation set: the neuron's top activating texts (labeled positive) + an equal number of fixed random zero-activation texts (labeled negative). The score is the F1 of the yes/no annotations against those labels. The annotator answers "No" when uncertain and sees ONLY one text + your description, so the description must be a self-contained, objectively checkable property of a single text.

YOUR OBJECTIVE: maximize official-set F1 — pure hill-climbing toward perfect discrimination (Yes on every positive, No on every negative), i.e. F1 = 1.0. Prefer the cleanest description that achieves a given score, but coverage beats elegance: if the neuron's concept is genuinely disjunctive, write the disjunction.
...
1. run_python(code): persistent Python sandbox (state survives across calls). Preloaded:
   ...
   - official_pos, official_neg: the index lists of the official evaluation set
...
2. score_descriptions(descriptions, indices=None): runs the official annotator.
   - indices=None (default): scores on the FULL official 100-example set — this is the reported metric.
   - indices=[...]: scores on any text indices you choose (labels = activation>0); use this to cheaply sanity-check candidates on small or adversarial samples you picked in python.
   Returns precision/recall/f1/correlation and the misclassified indices (false_positive_idx/false_negative_idx) — inspect them with show().
   BUDGET: {budget} total annotator calls for this neuron (one call = one text x one description). A full official scoring costs 100. Repeated (text, description) pairs are cached and FREE. Max {max_per_call} descriptions per call.

REQUIRED PROCESS:
1. Read the official positives across their range (top, middle, bottom of the 50) and several official negatives, plus zero-activation texts that are semantically close to the positives.
2. Form hypotheses at multiple specificity levels; keyword-probe them in python first.
3. Your FIRST score_descriptions call must include this previous researcher's description VERBATIM as one candidate (it is your floor):
   "{baseline_desc}"
4. After each official scoring, read the misclassified texts, diagnose, and revise: false positives mean too broad; false negatives mean too narrow or a missing disjunct.
5. Keep iterating while budget remains and you are improving. Stop only at F1 = 1.0, or no improvement over 3 consecutive attempts, or budget nearly exhausted (save enough for a final official scoring). Attack EVERY residual misclassification: read it, understand why the annotator got it wrong, and patch the description.
...
When done, return FinalResult with the best description EXACTLY as scored on the official set (verbatim) and a one-paragraph rationale."""


def build_agent_for_neuron(j, texts, stars, acts, emb, state):
    act = acts[:, j]
    o_pos, o_neg = test_indices(acts, j)
    # Scorer uses these closure-captured tuples; the sandbox gets separate
    # copies, so nothing the agent executes can alter what gets scored.
    o_pos_locked, o_neg_locked = tuple(o_pos), tuple(o_neg)
    ns = {
        "np": np, "texts": texts, "stars": stars, "act": act,
        "all_acts": acts, "emb": emb, "top": np.argsort(-act),
        "official_pos": list(o_pos), "official_neg": list(o_neg),
    ...
    def score_descriptions(descriptions: list[str], indices: list[int] | None = None) -> str:
        """Score descriptions with the official annotator. indices=None -> full official set (reported metric); otherwise your chosen text indices (labels = activation>0)."""
        descriptions = list(dict.fromkeys(d.strip() for d in descriptions if d.strip()))[:MAX_DESCS_PER_CALL]
        official = indices is None
        if official:
            pos_idx, neg_idx = list(o_pos_locked), list(o_neg_locked)
        else:
        ...
        state["spent"] += cost
        # ONE batched annotate call across ALL descriptions x texts (max parallelism).
        idx = list(pos_idx) + list(neg_idx)
        ev_texts = [truncate_text(texts[i], MAX_WORDS) for i in idx]
        ...
        for d in descriptions:
            ...
            m = compute_metrics(ann, labels, act[idx])
            ...
            if official:
                state["scored"][d] = {k: v for k, v in m.items() if isinstance(v, float)}
            out[d] = {k: (round(v, 3) if isinstance(v, float) else v) for k, v in m.items()}
        resp = {"results": out, "evaluated_on": f"FULL official set ({len(pos_idx)}+/{len(neg_idx)}-)" if official else f"custom sample ({n_texts} texts) - NOT the official metric",
    ...
    instructions = INSTRUCTIONS_TEMPLATE.format(
        ...
        n_total=len(texts), dataset_desc=DATASET_DESC, baseline_desc=baseline_desc)
    state["instructions"] = instructions
    agent = Agent(name=f"neuron-{j}-v5", instructions=instructions,
    ...
    return agent, (o_pos, o_neg), cache


async def optimize_neuron(j, texts, stars, acts, emb, sem):
    async with sem:
        ...
        agent, (o_pos, o_neg), cache = build_agent_for_neuron(j, texts, stars, acts, emb, state)
        print(f"[n{j}] v5 starting", flush=True)
        ...
        if claimed and claimed not in state["scored"]:
            m = score_description(claimed, o_pos, o_neg, texts, acts[:, j],
                                  cache_path=cache, return_errors=False)
            m.pop("correlation", None)
        ...
        print(f"[n{j}] v5 done: official_f1={final_m['f1']:.3f} "
              f"(spent {state['spent']}) :: {final_desc[:90]}", flush=True)
        return j, record
...
async def main():
    ...
    print("v5 optimizing neurons:", neurons, flush=True)
    sem = asyncio.Semaphore(N_CONCURRENT)
    results = await asyncio.gather(*[
        optimize_neuron(j, texts, stars, acts, emb, sem) for j in neurons],
    ...
    for j, rec in results:
        ...
        summary[str(j)] = {"description": rec["final_description"],
                           "test_corr": rec["official_metrics"]["correlation"],
                           "test_f1": rec["official_metrics"]["f1"],
                           "source": "v5-single-set",
                           "annotator_calls": rec["annotator_calls_spent"]}
        b = baselines[str(j)]["test_metrics"]["correlation"]
        print(f"n{j}: official {b:.3f} -> {summary[str(j)]['test_corr']:.3f}", flush=True)
    with open(os.path.join(ART, f"{RUN_NAME.replace('/', '_')}_results.json"), "w") as f:
        json.dump(summary, f, indent=2)
    cs = [v["test_corr"] for v in summary.values()]
    fs = [v["test_f1"] for v in summary.values()]
    print(f"AGENT-v5 mean official correlation: {np.mean(cs):.3f}, mean F1: {np.mean(fs):.3f}")
